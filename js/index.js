...
buttonSwitchTheme.addEventListener("click", () => {
  ...
  if (darkThemeInUse) {
    imgButtonSwitchTheme.setAttribute("src", "./images/solrock (1).png");
  } else {
    imgButtonSwitchTheme.setAttribute("src", "./images/lunatone.png");
  }
});

const pokemonImg = document.querySelectorAll(
  ".gif-xs, .gif-s, .gif-m, .gif-b, .gif-xb, .gif-ekans, .gif-charmeleon"
);

for (let i = 1; i <= 26; i++) {
  const buttonSwitchSprite = document.getElementById(
    `button-switch-sprite-${i}`
  );

  buttonSwitchSprite.addEventListener("click", () => {
    const spriteInUse = body.classList.contains(`./images/${i}.gif`);

    body.classList.toggle(`./images/${i}.gif`);

    if (spriteInUse) {
      pokemonImg[i - 1].setAttribute("src", `./images/${i}.gif`);
    } else {
      const audio = new Audio(
        "./audio/src_audio_src_audio_Shiny Sparkle Sound Effect Generation 3 (1).mp3"
      );
      audio.play();

      const sparkleImg = document.createElement("img");
      sparkleImg.src = "./images/sparkle.gif";
      ...
      pokemonImg[i - 1].setAttribute("src", `./images/${i}-shiny.gif`);
